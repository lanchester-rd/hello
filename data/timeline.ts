/* ─── Timeline data model & seed data  (V3) ─── */
...
export function gdrive(fileId: string, width = 1920): string {
...
export interface TimelineMedia {
  id: string;
  /** "image" now; structure ready for "video" later */
  type: "image";
  src: string;
  alt: string;
  caption?: string;
  /** Parallax speed multiplier – 0 = static, 1 = full scroll speed */
  depth: number;
  /** CSS object-position hint, e.g. "center top" */
  position?: string;
  /** CSS aspect-ratio hint, e.g. "16/9" */
  aspectHint?: string;
  /** Background opacity override (0–1) */
  opacityHint?: number;
  /** Tailwind gradient class for overlay, e.g. "from-background/80 to-transparent" */
  overlayGradient?: string;
  /** V3: explicit ordering within an event */
  sortOrder?: number;
}

/** @deprecated Use TimelineMedia — kept for V1 compat */
export type TimelineImage = TimelineMedia;
...
export interface TimelineEra {
  id: string;
  label: string;
  /** First & last event ids that belong to this era */
  startEventId: string;
  endEventId: string;
}
...
export interface TimelineEvent {
  ...
  /** Era id this event belongs to */
  era?: string;
  /** Short milestone label shown on the track, e.g. "Founded" */
  milestone?: string;
  links?: TimelineLink[];
  /** V2 rich media array — also exposed as `images` for V1 compat */
  media: TimelineMedia[];
  /** @deprecated Alias kept for V1 — always identical to `media` */
...
/* ─── Helper: build an event with `images` alias ─── */

function evt(
  e: Omit<TimelineEvent, "images"> & { media: TimelineMedia[] }
): TimelineEvent {
  return { ...e, images: e.media };
}

/* ─── Seed data — grounded biography dataset (V3) ─── */
...
export const timelineEvents: TimelineEvent[] = [
  ...
  evt({
    ...
    summary:
      "Seven years at Sandbach School, Cheshire — where a passion for design, making, and systems thinking started early. Design Technology and Art were the spark that lit everything that followed.",
    body:
    ...
    tags: ["education", "design-technology", "design-thinking", "foundations", "cheshire"],
  ...
  evt({
    ...
    theme: "dark",
    summary:
      "A week inside Bentley Motors' Crewe factory at sixteen — watching world-class precision engineering and luxury manufacturing in real life. The moment design intent meets manufacturing reality, and nothing is ever the same again.",
    body:
      "Most design students learn production processes from a textbook. This was different. Standing inside Bentley's Crewe factory — one of the most precise manufacturing environments on the planet — made one thing instantly clear: design decisions have consequences at every stage of production. A curve that looks perfect on screen can become a nightmare on the factory floor. Quality isn't an opinion; it's a system. This early exposure to end-to-end product lifecycle thinking, manufacturing constraints, and quality management principles became a permanent reference point for every product and design role that followed.",
    achievements: [
    ...
    tags: ["work-experience", "automotive", "manufacturing", "product-lifecycle", "quality-management", "bentley"],
    status: "published",
    media: [
      {
        id: "bwx-1",
  ...
  /* ════════════════  EARLY FORMATION (parallel threads: coaching & freelance)  ════════════════ */
  ...
  evt({
    ...
    theme: "dark",
    summary:
      "Seven years coaching swimming across all ages and abilities — running in parallel with school, university, and early career. Teaching people to do something hard, clearly and confidently, turned out to be excellent training for product design and team leadership.",
    body:
      "Coaching is a design challenge. You take something complex, break it into steps, communicate it clearly, and adjust the approach based on how the person in front of you is responding. That's also exactly how great products are built and great teams are led. Seven years of qualified swimming instruction — while studying, while freelancing, while building skills — developed communication, stakeholder empathy, structured coaching, and adaptive facilitation skills that no classroom delivers. It also funded the journey through design education without compromise.",
    achievements: [
    ...
    tags: ["coaching", "communication", "instructional-design", "stakeholder-empathy", "facilitation"],
    status: "published",
    media: [
      {
        id: "swim-1",
  ...
  evt({
    ...
    links: [{ label: "About Lanchester R&D", href: "/about" }],
    status: "published",
    media: [
      {
        id: "lef-1",
  ...
  evt({
    ...
    theme: "dark",
    summary:
      "A collaborative studio experience in 2013, bridging the gap between undergraduate study and the Coventry Masters. Real briefs, real critique, real deadlines — sharpening the move from solo freelancer to professional design collaborator.",
    body:
      "Independent freelance work builds execution skills. Collaborative studio work builds something different: the ability to design with others, defend a point of view under critique, and align a team around a shared creative direction. One Percent Studio was the transition from solo practice to collaborative design — working alongside other designers on live client briefs, presenting work, and learning how to navigate the gap between what a designer thinks is right and what a client needs to hear. These professional collaboration and stakeholder communication skills became essential in every team role that followed.",
    achievements: [
    ...
    tags: ["design-studio", "collaboration", "design-critique", "stakeholder-communication", "professional-development"],
    status: "draft",
    media: [
      {
        id: "ops-1",
  ...
  evt({
    ...
    quote:
      "Great design isn't subjective — it's defensible, measurable, and grounded in real insight.",
    links: [
      {
  ...
  evt({
    ...
    theme: "light",
    summary:
      "Invited back to Sandbach School as Designer in Residence directly after the Coventry Masters — mentoring students on design projects and bringing real-world product thinking into the classroom.",
    body:
      "Coming back to the school that started everything — this time as the professional in the room — was a powerful experience. As Designer in Residence, the role was to bring the gap between academic design education and professional product practice into the open. Mentoring students through live design projects, introducing user research principles, manufacturing realities, and iterative design thinking into their process. It reinforced one of the most important leadership skills: making complex, expert knowledge accessible to someone encountering it for the first time. The ability to mentor, coach, and transfer design thinking across experience levels has been central to every team leadership role since.",
    achievements: [
  ...
  /* ════════════════  PRODUCT & INDUSTRIAL DESIGN ROLES  ════════════════ */
  ...
  evt({
    ...
    theme: "dark",
    summary:
      "First professional industrial design role at Elektron Technology, Cambridge — designing medical devices, packaging systems, and optimising manufacturing processes in a highly regulated, precision-critical environment.",
    body:
  ...
  /* ════════════════  LANCHESTER R&D FOUNDATION (Phase 2)  ════════════════ */
  ...
  evt({
    ...
    links: [
      { label: "About Lanchester R&D", href: "/about" },
  ...
  /* ════════════════  PRODUCT & INDUSTRIAL DESIGN (continued)  ════════════════ */
  ...
  evt({
    ...
    theme: "dark",
    summary:
      "Product and graphic designer at RATstands — a specialist music accessories brand whose products perform on concert stages at the world's top venues and touring productions. Designing objects where quality, reliability, and precision are non-negotiable.",
    body:
  ...
  evt({
    ...
    links: [
      { label: "bunq", href: "https://www.bunq.com" },
    ],
    status: "published",
    media: [
      {
  ...
  evt({
    ...
    links: [
      { label: "IRECKONU", href: "https://www.ireckonu.com" },
    ],
    status: "published",
    media: [
      {
  ...
  evt({
    ...
    summary:
      "Joined xpate as employee #8 and Head of Product Design — and built the entire product design function from zero. Style guide, design processes, product workflows, and a live product in the hands of 100 real users. In 205 days.",
    body:
      "Joining a seed-stage startup as employee #8 means one thing: everything needs to be built. At xpate, a fintech payments platform, the design function didn't exist before day one. The job was to create it — fast, properly, and in parallel with a team that was simultaneously building the engineering stack, the sales pipeline, and the compliance framework. A comprehensive product style guide was created from scratch. Design-to-development workflows were established. Sprint planning and design review processes were introduced. Product decisions were made, owned, and shipped. The launch milestone — 100 active users — was hit in 205 days. The xpate experience is the clearest proof point that speed and quality are not opposites: with the right design systems, process discipline, and stakeholder alignment, both are achievable together.",
    achievements: [
    ...
    links: [
      { label: "xpate", href: "https://www.xpate.com" },
    ],
    status: "published",
    media: [
      {
  ...
  evt({
    ...
    links: [
      { label: "About Lanchester R&D", href: "/about" },
      { label: "LinkedIn", href: "https://www.linkedin.com/in/timdraper" },
    ],
    status: "published",
    media: [
      {
  ...
  evt({
    ...
    summary:
      "Co-founded The iMediate Foundation — a co-parenting platform built to reduce conflict and improve communication for separated families. Scheduling tools, WhatsApp automation, ICS calendar export, Firebase backend, and a real go-to-market strategy. Built to matter.",
    body:
  ...
  evt({
    ...
    links: [
      { label: "ShootAtlas", href: "https://www.shootatlas.com" },
    ],
    status: "published",
    media: [
      {
  ...
  evt({
    ...
    summary:
      "Founded Guerilla Kitchen — a community food operations platform that replaced manual allocation chaos with a fairness-based algorithm. No-shows dropped from ~7 to 1 per session. Volunteer admin time cut dramatically. Multi-client architecture built for scale from day one.",
    body:
    ...
    links: [
      { label: "Guerilla Kitchen", href: "https://www.guerillakitchen.org" },
    ],
    status: "published",
    media: [
      {
...
/** All unique categories across events */
export const timelineCategories: string[] = [
